...
> 约束：**中文词内无空格**（但原始文本可能包含空格/换行/混排内容），**不得破坏原始文本结构**（英文/数字/URL/数学符号/标点保持原样与原位置）。
...
* 输出拼音默认**不插入新空格**，除非输入中原本就有空格/换行。
* 对混排：英文单词、数字、网址 URL、数学符号、标点、emoji 等 **原样保留**。
...
> 重要：输出文本中不应为了可读性而新增空格；必须保持输入中已有的空格/换行/标点的相对位置与字符内容（中文被替换成拼音是预期变化）。
...
```json
{
  ...
  "spans": [
    ...
    },
    {
      "span_id": "S1",
      "type": "protected",
      "kind": "url|latin|number|punct|space|symbol|emoji|other",
      "start": 6,
      "end": 7,
      "text": "，"
    }
...
1. 回退为“按字切分”：每个汉字一个 token，`upos="X"`, `xpos="UNK"`, `ner="O"`。
2. 在 report 中记录 `llm_invalid=true`，方便后续排查。
...
```
Token("银行") 
  -> 1. word.json 词表匹配: "银行" -> "yín háng" (命中，直接使用)
  -> 2. 若未命中: char_base.json 字表: "银"->"yin2", "行"->"hang2" (单音字)
...
* 结构助词（轻声）：`upos=PART` 或 `xpos=DEC/DEG/DER`（若使用 CTB 细分）

  * 例：`跑得快` → `pǎodekuài`
* 动词“得到”（dé2）：token text 精确等于 `得到/得出/得知/...` 或 `upos=VERB` 且后接宾语

  * 例：`得到答案` → `dédáodáàn`
* 情态/必须（děi3）：`得去/得做/得赶紧`，常见为 `upos=AUX` 或 `self.text` 匹配 `得(去|做|赶紧|马上)`

  * 例：`我得去` → `wǒděiqù`

### 8.2 “的” (de5 / dí2 / dì4)

* 结构助词（轻声）：`upos=PART` → `de5`
* `目的`：token text = `目的` → `di4`
* `的确`：token text = `的确` → `di2`

### 8.3 “重” (zhòng4 / chóng2)

* `重新/重复/重来/重启/重做`：表示“再次”倾向 `chóng2`

  * match: `self.text_in=["重新","重复","重来","重启","重做"]` → `chong2`
* 形容词“重要/沉重/重视/重心/重负”倾向 `zhòng4`

  * match: `self.upos_in=["ADJ","VERB","NOUN"]` 且 `self.contains=["重"]` 并命中词表 → `zhong4`

### 8.4 “行” (xíng2 / háng2)

* 行业/银行/同行/本行/支行：`háng2`

  * match: `self.text_in=["银行","行业","同行","本行","支行"]`
* 动词“行走/行使/行不行/可行”：`xíng2`

  * match: `self.upos_in=["VERB","AUX"]` 或 `self.text_in=["可行","行走","行使"]`

### 8.5 “长” (cháng2 / zhǎng3)

* 形容词“长短/很长/延长/长度”：`cháng2`

  * match: `self.upos_in=["ADJ"]` 或 `prev.upos_in=["ADV"]`（很/更/太）且 self 含 “长”
* 名词/职位“校长/队长/行长/部长/家长”：`zhǎng3`
...
* 地名中的特殊读音（如"厦"在"厦门"）：`ner=LOC` 辅助判断
  ...
  * match: `self.ner_in=["LOC"]` 且 `self.contains=["厦"]` → `xia4`
...
* 规则选择使用 `pinyin_with_tone_number`（如 `hang2`, `de5`）。
* 最终输出使用 tone marks（如 `háng`, `de`（轻声无音标））。

### 9.2 tone marks 转换规则（实现要求）

实现一个 `numbered_to_marked(pinyin: str) -> str`：
...
* 声调位置规则（标准拼音）：
  ...
  4. 否则在最后一个元音上标
  5. 特例：`iu` 标 `u`，`ui` 标 `i`
* `ü` 处理：内部可用 `v` 表示 `ü`，最终输出必须为 `ü`（带声调时为 `ǖǘǚǜ`）。
* 轻声（5）不加音标（`de5` → `de`）。
  ...
  "rules": [
    {
      ...
      "priority": 100000,
      "description": "user override: 行(行长)=hang2",
      "match": {
...
* 对 `protected` span：原样输出 `span.text`
* 对 `han` span：按原字符顺序输出其拼音（每字一个音节），**不额外插空格**
...
```
yínhánghángzhǎngchóngxīnyíngyè，OpenAI API v2.0：https://openai.com
```
...
* in: `银行行长重新营业`
  out: `yínhánghángzhǎngchóngxīnyíngyè`

3. 得：

* in: `他得去得到答案`
  out: `tāděiqùdédáodáàn`

4. 混排：

* in: `细说OpenAI的API v2.0：https://openai.com`
  out: `xìshuōOpenAIdeAPI v2.0：https://openai.com`
